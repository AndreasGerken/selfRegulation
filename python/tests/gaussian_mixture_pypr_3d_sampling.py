...
ax.scatter(X[:500,0].tolist(), X[:500,1].tolist(), X[:500,2].tolist(), alpha=0.4)

# Expectation-Maximization of Mixture of Gaussians
cen_lst, cov_lst, p_k, logL = gmm.em_gm(X, K = 4, max_iter = 400, verbose = True, iter_call = None)
...
for i in range(len(samples)):
    ax.scatter(samples[i][0] ,samples[i][1] ,samples[i][2], c = 'r', alpha = 0.6, edgecolors='none')
